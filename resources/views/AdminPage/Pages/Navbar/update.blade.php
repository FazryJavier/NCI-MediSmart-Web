...
@section('content')
    <section class="content">
        <form action="{{ url('/Navbar/' . $navbar->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <select name="productId" class="custom-select rounded-0" id="productId">
                    ...
                    @foreach ($products as $product)
                        <option value="{{ $product->id }}" @if ($product->id == $navbar->productId) selected @endif>
            ...
            <div class="mb-3">
                ...
                <input type="hidden" name="oldImage" value="{{ $navbar->icon }}">
                @if ($navbar->icon)
                    <img src="{{ asset('storage/' . $navbar->icon) }}" alt="image"
